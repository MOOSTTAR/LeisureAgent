...
4. **用餐地点尽量不重复**：如果行程包含多顿饭（跨天或同天多餐），尽量安排不同的餐厅。但如果用户明确指定了要吃同一家（如"两天都吃火锅"、"还是去那家"），则尊重用户意愿。用户没说的话默认选不同的
5. 考虑地点间的距离，用 |x1-x2|+|y1-y2| 计算两点间距离。travel_mode 决定出行耗时：
...
## 输出前自我核查清单（每次生成方案后必须逐项自查，确认无误后再输出 JSON）
1. □ items 的 day_num 是否覆盖了 1 到 {day_count} 所有值？（多日行程每天至少1项，单日全部为1）
2. □ day_label 是否从用户输入中提取了正确的星期标签？
3. □ 每天的 arrive_time/leave_time 是否独立递增、无时间重叠？
4. □ 所有 location_id 是否均来自候选列表（未编造）且 available=true？
5. □ 用餐地点是否尽量不重复？（用户明确指定除外）
6. □ title 是否具体、有辨识度？（非泛化标题）

**如果自查清单有任何一项未通过，请在输出前修正。不要输出有错误的 JSON！**
"""
...
def format_compose_prompt(
    ...
    if day_count > 1:
        day_labels_hint = ""
        # 尝试从用户输入中提取星期信息，给 LLM 更具体的指引
        import re as _re
        days_found = _re.findall(r'(周六|周日|星期六|星期日)', user_input)
        if days_found:
            day_labels_hint = (
                f"\n从用户输入中提取到的星期：{'、'.join(days_found[:day_count])}。"
                f"\n请严格按照这个顺序分配 day_label。"
            )
        multi_day_note = (
            f"\n\n{'='*50}\n"
            f"【最高优先级指令 · 违反即作废】检测到跨天行程（{day_count} 天）！\n\n"
            f"你必须生成恰好 {day_count} 天的活动，每天至少 1 项。\n"
            f"❌ 常见错误（绝对禁止）：把所有活动都设为 day_num=1，导致其他天空白！\n"
            f"✅ 正确做法：仔细阅读用户原始输入，按日期拆分活动：\n"
            f"   - 第1天的活动 → day_num=1\n"
            f"   - 第2天的活动 → day_num=2\n"
            f"{f'   - ...至第{day_count}天' if day_count > 2 else ''}\n"
            f"每天独立设置 arrive_time/leave_time。第1天从用户指定时间开始，后续天从上午10:00开始。\n"
            f"{day_labels_hint}\n"
            f"请在输出前自查：items 数组中是否出现了 day_num=1..{day_count} 的所有值？\n"
            f"{'='*50}"
        )
