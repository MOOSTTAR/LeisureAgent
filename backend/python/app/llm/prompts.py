...
- "new_plan"：请求生成一个周末活动方案（"出去玩"、"安排"、"规划"、"一日游"）
- "feedback"：对已有方案提出修改意见（换、去掉、改、不要）
...
12. **标题多样性**：title 必须根据每次的具体行程生成，融入实际地点名和场景，禁止使用\"周末活动方案\"、\"半日游方案\"等泛化标题。好的标题示例：\"蓝色港湾亲子半日游\"、\"三里屯火锅+逛街闺蜜周末\"、\"故宫+景山两日文化之旅\"
"""
