...
def search(
    ...
    if hall_type:
        clauses.append("hall_type = ?")
        params.append(hall_type)
    if free_entry is not None:
        if free_entry:
            clauses.append("ticket_type = 0")
        else:
            clauses.append("ticket_type = 1")
    ...
    rows = conn.execute(
        f"SELECT * FROM exhibition_hall{where} ORDER BY id LIMIT ? OFFSET ?",
        [*params, limit, offset],
    ).fetchall()
    return [dict(r) for r in rows]
...
def count(
    ...
    where = " WHERE " + " AND ".join(clauses) if clauses else ""
    row = conn.execute(f"SELECT COUNT(*) FROM exhibition_hall{where}", params).fetchone()
    return row[0]
